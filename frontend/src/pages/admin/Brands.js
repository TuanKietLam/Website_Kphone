...
import { getAdminBrands, createBrand, updateBrand, deleteBrand } from '../../utils/api';
...
function AdminBrands() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['brands'] }); // Invalidate client cache
      setShowForm(false);
      resetForm();
    },
  });
  ...
  const handleDelete = (brandId) => {
    if (window.confirm('Bạn có chắc muốn xóa thương hiệu này? Lưu ý: Không thể xóa thương hiệu đang có sản phẩm.')) {
      deleteMutation.mutate(brandId, {
        onError: (error) => {
          alert(error.response?.data?.error || 'Không thể xóa thương hiệu này');
        }
      });
    }
  };
  ...
  return (
    <div style={styles.page}>
      <div style={styles.header}>
        <h1 style={styles.title}>Quản lý thương hiệu</h1>
        ...
        <table style={styles.table}>
          ...
          <tbody>
            {brandsData?.brands?.map((brand, index) => (
              ...
              >
                <td style={styles.tableBodyCell}>{brand.id}</td>
                <td style={{...styles.tableBodyCell, fontWeight: '500'}}>{brand.name}</td>
                <td style={styles.tableBodyCell}>
                  <span style={{color: brand.description ? '#212529' : '#6c757d'}}>
                    {brand.description || '-'}
                  </span>
                </td>
                <td style={styles.tableBodyCell}>
                ...
                <td style={styles.tableBodyCell}>
                  <span style={{
                    ...styles.statusBadge,
                    backgroundColor: brand.status === 'active' ? '#28a745' : '#6c757d',
                  }}>
